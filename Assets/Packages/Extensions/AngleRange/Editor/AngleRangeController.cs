...
namespace Gameplay.AngleRange.Editor
{
    ...
    public class AngleRangeController
    {
        ...
        public int selectedIndex => cache.selectedIndex;
        ...
        {
            var index = AngleRangeUtility.GetRangeIndexFromAngle(cache.angleRanges, cache.previewAngle);
            SelectIndex(index);
        }
        ...
        private void HandleRemoveRange()
        {
            if (view.DoRemoveRange())
            {
                ...
                onRangeRemoved?.Invoke(cache.selectedIndex);
                SelectIndex(-1);
            }
        }
        ...
        private void CreateRangeAtAngle(float angle)
        {
            ...
            if (GetNewRangeBounds(angle, out start, out end))
            {
                ...
                SetSelectedIndexFromPreviewAngle();
                Debug.Log("Added range");
                onRangeAdded?.Invoke(cache.selectedIndex);
            }
        }
