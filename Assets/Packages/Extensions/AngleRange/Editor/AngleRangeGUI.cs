    ...
    public class AngleRangeGUI
    {
        ...
        public static void AngleRangeField(Rect rect, int leftHandleID, int rightHandleID, ref float start, ref float end, float angleOffset, float radius, bool snap, bool drawLine, bool drawCircle, Color rangeColor)
        {
            ...
            if (Event.current.type == EventType.Repaint)
            {
                ...
                AngleRangeUtility.DrawSolidArc(rect.center, Vector3.forward, Quaternion.AngleAxis(start + angleOffset, Vector3.forward) * Vector3.right, range, radius, kRangeWidth);
                Handles.color = color;
            }
            ...
            start = AngleField(rect, leftHandleID, start, angleOffset, new Vector2(-3.5f, -7.5f), start + angleOffset + 90f, handleSize, radius, snap, drawLine, drawCircle, AngleRangeUtility.RangeLeftCap);
            ...
            end = AngleField(rect, rightHandleID, end, angleOffset, new Vector2(3.5f, -7.5f), end + angleOffset + 90f, handleSize, radius, snap, drawLine, false, AngleRangeUtility.RangeRightCap);
        ...
        public static float AngleField(Rect rect, int id, float angle, float angleOffset, Vector2 handleOffset, float handleAngle, float handleSize, float radius, bool snap, bool drawLine, bool drawCircle, Handles.CapFunction drawCapFunction)
        {
            ...
            if (GUI.enabled)
            {
                ...
                Vector2 posNew = AngleRangeUtility.Slider2D(id, pos, rotation * handleOffset, rotation, handleSize, drawCapFunction);
