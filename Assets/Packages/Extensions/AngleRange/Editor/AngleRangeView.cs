...
public enum AngleRangeAction
    ...
    public class AngleRangeView : IAngleRangeView
    {
        ...
        {
            return Mathf.RoundToInt(AngleRangeUtility.PosToAngle(Event.current.mousePosition, rect.center, -angleOffset));
        }
        ...
        private void LayoutCreateRange(Rect rect, float angleOffset, float radius)
        {
            if (Event.current.type == EventType.Layout)
            {
                ...
                var distance = AngleRangeUtility.DistanceToArcWidth(mousePosition, rect.center, 0f, 360f, radius, AngleRangeGUI.kRangeWidth, angleOffset);
        ...
        public bool DoAngleRange(int index, Rect rect, float radius, float angleOffset, ref float start, ref float end, bool snap, bool disabled, Color gradientMin, Color gradientMid, Color gradientMax)
        {
            ...
            if (Event.current.type == EventType.Layout)
            {
                var distance = AngleRangeUtility.DistanceToArcWidth(Event.current.mousePosition, rect.center, start, end, radius, AngleRangeGUI.kRangeWidth, angleOffset);
        ...
        {
            EditorGUI.BeginChangeCheck();
            newAngle = AngleRangeGUI.AngleField(rect, m_SelectorControlID, angle, angleOffset, Vector2.down * 7.5f, angle, 15f, radius - AngleRangeGUI.kRangeWidth, true, true, false, AngleRangeUtility.PlayHeadCap);
        ...
        {
            if (Event.current.type == EventType.Repaint)
                AngleRangeUtility.DrawRangeOutline(start, end, angleOffset, rect.center, radius, AngleRangeGUI.kRangeWidth - 1f);
        }
